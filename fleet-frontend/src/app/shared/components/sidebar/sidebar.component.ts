...
@Component({
  ...
  template: `
    <aside class="sidebar" [class.collapsed]="collapsed()">
      ...
      <div class="sidebar-logo">
        ...
        @if (!collapsed()) { <span class="logo-text">FleetMgr</span> }
      </div>
      ...
      <nav class="sidebar-nav">
        ...
        <a routerLink="/reports" routerLinkActive="active" class="nav-item" [attr.title]="collapsed() ? 'Reports' : null">
          <span class="nav-icon">📊</span>@if (!collapsed()) {<span>Reports</span>}
        </a>
        ...
        @for (item of fleetItems; track item.route) {
          <a [routerLink]="item.route" routerLinkActive="active" class="nav-item" [attr.title]="collapsed() ? item.label : null">
            <span class="nav-icon">{{ item.icon }}</span>@if (!collapsed()) {<span>{{ item.label }}</span>}
          </a>
        }
        ...
        @if (auth.hasRole('Admin')) {
          ...
          <a routerLink="/users" routerLinkActive="active" class="nav-item" [attr.title]="collapsed() ? 'User Management' : null">
            <span class="nav-icon">👥</span>@if (!collapsed()) {<span>User Management</span>}
          </a>
        }
      </nav>
      ...
      <div class="sidebar-footer">
        <button class="nav-item logout-btn" (click)="auth.logout()" [attr.title]="collapsed() ? 'Logout' : null">
          <span class="nav-icon">⇥</span>@if (!collapsed()) {<span>Logout</span>}
        </button>
      </div>
    </aside>
  `,
  styles: [`
    ...
    .sidebar-logo { display:flex; align-items:center; gap:10px; padding:20px 20px 16px; border-bottom:1px solid var(--sidebar-border); white-space:nowrap; }
    ...
    .user-arrow { margin-left:auto; color:var(--sidebar-muted); font-size:18px; font-weight:300; transition:transform 0.15s, color 0.15s; }
    ...
    .sidebar-nav { flex:1; padding:12px 12px 0; display:flex; flex-direction:column; gap:2px; overflow-y:auto; }
